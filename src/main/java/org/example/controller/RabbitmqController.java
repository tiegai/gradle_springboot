...
public class RabbitmqController {
    ...
    @GetMapping("/sendDirectAckMessage1")
    public String sendDirectMessageAck() {
        String messageId = String.valueOf(UUID.randomUUID());
        String messageData = "test message, hello ack11!";
        String createTime = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        Map<String, Object> map = new HashMap<>();
        map.put("messageId", messageId);
        map.put("messageData", messageData);
        map.put("createTime", createTime);
        //将消息携带绑定键值：TestDirectRouting 发送到交换机TestDirectExchange
        rabbitTemplate.convertAndSend("TestDirectAckExchange1", "TestDirectAckRouting1", map);
        return "ok";
    }

    @GetMapping("/sendDirectAckMessage2")
    public String sendDirectMessageAck2() {
        String messageId = String.valueOf(UUID.randomUUID());
        String messageData = "test message, hello ack22!";
        String createTime = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        Map<String, Object> map = new HashMap<>();
        map.put("messageId", messageId);
        map.put("messageData", messageData);
        map.put("createTime", createTime);
        //将消息携带绑定键值：TestDirectRouting 发送到交换机TestDirectExchange
        rabbitTemplate.convertAndSend("TestDirectAckExchange2", "TestDirectAckRouting2", map);
        return "ok";
    }
    ...
    public void sendMsg(@PathVariable String message){
        //System.out.println("当前时间：{}, 发送一条消息：{} 给两个TTL队列", new Date(), message);
...
/*    @GetMapping("/sendDelayedMsg/{message}/{delayedTime}")
    public void sendMsg(@PathVariable String message, @PathVariable Integer delayedTime){
        ...
        //System.out.println("当前时间：{}, 发送一条时长为 {}ms的消息给延时队列 ==> {}", new Date(), delayedTime, message);
